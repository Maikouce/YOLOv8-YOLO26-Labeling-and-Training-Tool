...
export class UIManager {
    ...
    injectCustomStyles() {
        ...
        style.textContent = `
            ...
            /* 加大勾选框 */
            .item-check-wrapper input[type="checkbox"] {
                width: 18px;
                height: 18px;
                cursor: pointer;
                margin: 0;
    ...
    async handleDatasetUpload(e) {
        ...
        this.dom.uploadProgressBar.value = 0;
        this.dom.uploadProgressBar.max = 100;

        // --- 配置 ---
        const BATCH_SIZE = 20; // 每批次上传 20 张，保证稳定
        ...
        try {
            ...
            for (let i = 0; i < totalFiles; i += BATCH_SIZE) {
                ...
                // 更新状态文字
                this.dom.uploadStatusText.textContent = `正在上传: ${i + 1} - ${Math.min(i + BATCH_SIZE, totalFiles)} / ${totalFiles}`;

                try {
                    // 调用 API 上传当前批次
                    // 注意：这里复用 apiClient.uploadDataset，它内部会自动附带 owner 和 taskName
                    const result = await this.bus.imageManager.apiClient.uploadDataset(formData);
                    successCount += chunk.length;
                } catch (err) {
                    console.error(`Batch upload failed at index ${i}:`, err);
                    failCount += chunk.length;
                    // 可以选择这里 break 终止，或者继续上传下一批。这里选择继续。
                }

                // 更新进度条
                processedCount += chunk.length;
                const percent = Math.round((processedCount / totalFiles) * 100);
                this.dom.uploadProgressBar.value = percent;
            }

            // 全部完成后
            this.dom.uploadStatusText.textContent = "处理完成";
            let msg = `上传完成。成功: ${successCount}`;
            if (failCount > 0) msg += `, 失败: ${failCount}`;

            this.showStatusMessage(msg, failCount > 0 ? "warning" : "success", 4000);
            ...
            console.error(err);
            this.showStatusMessage("上传流程异常: " + err.message, "error");
            this.dom.uploadStatusText.textContent = "异常终止";
        } finally {
            // 恢复 UI
            e.target.value = '';
            e.target.disabled = false;
            setTimeout(() => {
                this.dom.uploadProgressContainer.style.display = 'none';
            }, 3000);
        }
    }
    ...
    populateImageListForModal() {
        ...
        images.forEach((img, index) => {
            ...
            itemDiv.className = 'modern-list-item';

            // 2. 创建勾选区 (左侧)
            const checkWrapper = document.createElement('div');
            checkWrapper.className = 'item-check-wrapper';

            const checkbox = document.createElement('input');
            checkbox.type = 'checkbox';
            checkbox.value = img.name;
            checkbox.id = `img-select-${index}`;
            // 阻止勾选框区域的双击事件冒泡到父容器（虽然现在逻辑已经物理分离，但加上更保险）
            checkWrapper.addEventListener('dblclick', (e) => e.stopPropagation());
            ...
            infoWrapper.title = "双击跳转到此图片";

            const nameSpan = document.createElement('span');
            nameSpan.style.fontWeight = '500';
            nameSpan.textContent = `${index + 1}. ${img.name}`;
            ...
            // 4. 绑定逻辑：只有双击信息区才会跳转
            infoWrapper.addEventListener('dblclick', async (e) => {
                e.preventDefault();
                // 执行跳转
                this.bus.emit('showLoading', true, "正在保存并跳转...");
                try {
                    ...
                    this.showStatusMessage("跳转失败: " + err.message, "error");
                } finally {
                    this.bus.emit('showLoading', false);
                }
            });

            // 点击信息区时，自动勾选/取消勾选（这是一个可选的体验优化，如果你不想要，可以删掉这行）
            // infoWrapper.onclick = () => { checkbox.checked = !checkbox.checked; };

            // 5. 组装并挂载
            itemDiv.appendChild(checkWrapper);
            itemDiv.appendChild(infoWrapper);
            this.dom.imageListContainer.appendChild(itemDiv);
        });
    }
